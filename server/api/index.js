...
// Static files (⚠️ may not fully work in Vercel, but kept)
app.use("/images", express.static(path.join(projectRoot, "upload/images")));
app.use("/uploads", express.static(path.join(projectRoot, "uploads")));
app.use("/receipts", express.static(path.join(projectRoot, "upload/receipts")));
app.use("/profiles", express.static(path.join(projectRoot, "upload/profiles")));
app.use("/events", express.static(path.join(projectRoot, "upload/events")));
...
// ✅ Webhook helper (keep as is)
export const callOrderWebhook = async (webhookData) => {
  try {
    const response = await fetch(
      `${process.env.BASE_URL}/api/orders/webhook/payhere`,
      {
        method: "POST",
        headers: {
