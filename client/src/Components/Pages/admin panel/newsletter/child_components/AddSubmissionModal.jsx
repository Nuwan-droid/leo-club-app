...
const AddSubmissionModal = ({ isOpen, member, onSave, onCancel }) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (submissionData.title && submissionData.description) {
      const saveData = {
        ...submissionData,
        images: submissionData.images.map(img => img.file)
      };
      onSave(saveData);
  ...
  const handleCancel = () => {
    submissionData.images.forEach(img => URL.revokeObjectURL(img.url));
    setSubmissionData({ title: '', description: '', images: [], submissionType: 'newsletter_article' });
    onCancel();
  };
  ...
  return (
    <div className="fixed inset-0 bg-transparent bg-opacity-50 flex items-center justify-center z-50">
      <div className="relative bg-white rounded-lg p-6 w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto outline outline-1 outline-black shadow-xl">
        <button
        ...
        <h2 className="text-xl font-semibold mb-4 pr-8">Add Completed Submission</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            <textarea
              ...
              onChange={handleInputChange}
              rows={4}
              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-shadow resize-y"
                ...
                {submissionData.images.map((image, index) => (
                  <div key={index} className="relative group">
                    ...
                    <button
                      ...
                      className="absolute top-1 right-1 bg-red-500 text-white rounded-full w-6 h-6 flex items-center justify-center text-xs hover:bg-red-600 opacity-75 group-hover:opacity-100 transition-opacity"
                    >
                      ×
                    </button>
                  </div>
          ...
          <div className="flex justify-end space-x-3 mt-6">
            ...
            <button
              ...
              className="px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 disabled:bg-blue-300 disabled:cursor-not-allowed transition-colors focus:outline-none focus:ring-2 focus:ring-blue-400"
            >
              Add Submission
            </button>
          </div>
